...
func (c *configServiceClient) ListCodecs(ctx context.Context, in *ListCodecsRequest, opts ...grpc.CallOption) (*ListCodecsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ListCodecsResponse)
	err := c.cc.Invoke(ctx, ConfigService_ListCodecs_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _ConfigService_ListCodecs_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ListCodecsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(ConfigServiceServer).ListCodecs(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: ConfigService_ListCodecs_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(ConfigServiceServer).ListCodecs(ctx, req.(*ListCodecsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
